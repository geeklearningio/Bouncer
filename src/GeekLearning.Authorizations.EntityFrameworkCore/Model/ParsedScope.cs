namespace GeekLearning.Authorizations.EntityFrameworkCore.Model
{
    ...
    internal class ParsedScope
    {
        ...
        public IList<(Guid AuthorizationId, string Right)> ExplicitRightNames { get; private set; } = new List<(Guid AuthorizationId, string Right)>();

        public IList<(Guid AuthorizationId, string Right)> RightNames { get; private set; } = new List<(Guid AuthorizationId, string Right)>();

        public IList<(Guid AuthorizationId, string Right)> RightUnderNames { get; private set; } = new List<(Guid AuthorizationId, string Right)>();
        ...
        private ScopeRights ToScopeRights(Guid principalId)
        {
            var rightsOnScope = this.RightNames
                .Select(r => new Right(principalId, this.Scope.Name, r.Right, this.ExplicitRightNames.Contains(r), r.AuthorizationId))
                .ToList();

            var rightsUnderScope = this.RightUnderNames
                .Select(r => new Right(principalId, this.Scope.Name, r.Right, false, r.AuthorizationId))
                .ToList();
            ...
            Dictionary<Guid, ParsedScope> parsedScopes)
        {
            ...
            if (principalRightsPerScope.TryGetValue(scope.Id, out (Guid AuthorizationId, string Right)[] explicitRightNames))
            {
